...
* Чтобы зафиксировать изменения в репозитории используется команда:
    ...
    Если гит не дает сделать коммит и пишет ошибку о том, что какой то файл это блокирует или что то вроде того, то поможет команда:

    ```
    rm -f .git/index.lock
    ```

    Эта команда удалит файл (в данном случае index.lock) из репозитория и коммит будет возможен.
...
Чтобы поместить свой репозиторий на портал **GitHub**, нужно следовать шагам:
1. Переходим в свой профиль:
    
    ![Свой профиль](profile.jpg)
2. Далее идем к списку репозиториев:
    ![Репозитории](repositories.jpg)
3. Нажимаем на зеленую кнопку New:
    ![Новый](new.jpg)
4. Придумываем название нашего репозитория на **GitHab**-e.
5. Выбираем тип репозитория:
    * **Публичный** - открытый
    * **Приватный** - закрытый
6. Далее жмем:
...
Ура! Наш репозиторий на плотформе **GitHub** создан. Давайте же наполним его.

После создания удаленного репозитория нам дается на выбор 3 варианта его наполнения. Рассмотрим вариант, когда у нас на компьютере уже есть локальный репозиторий. Платформа **GitHub** подсказывает нам 3 команды, которые необходимы для перенесения локального репозитория в удаленный:

```
git remote add origin https://github.com/ВАШ_ПРОФИЛЬ/ВАШ_РЕПОЗИТОРИЙ.git
git branch -M main
git push -u origin main
```

Рассмотрим каждую команду подробнее:
* `git remote add origin <адрес>` - эта операция связывает ваш локальный и удаленный репозиторий.
* `git branch -M main` - эта команда меняет название вашей ветки на **main**, чтобы она называлась так же, как и в удаленном репозитории. Это нужно чтобы избежать ошибок.
* `git push -u origin main` - эта команда отправляет локальные изменения в удаленный репозиторий из ветки **main** в ветку **origin/main**. Оператор `-u` говорит ветке удаленного репозитория следовать за веткой локального репозитория... своеобразная операция связывания веток (это делается для дальнейших `push`-ей).

После связывания локального и удаленного репозитория команда `git push` будет просто обновлять информацию в удаленном репозитории.

_Возможно **Git** после последней операции предложит немного изменить команду. Нужно с этим согласиться и скопировать предложенную команду в строку ввода терминала._

Теперь наш локальный репозиторий связан с удаленным и они полностью идентичны.

Что мы можем сделать с удаленным репозиторием? Например мы можем склонировать репозиторий на другой компьютер командой `git clone <адрес репозитория>`.

* _Подробнее о команде `clone` читай в разделе 4 данной инструкции_.

Если вы создали публичный удаленный репозиторий, то в вашем проекте может кто-то поучавствовать, предложив свои изменения в вашем проекте. После того, как вы примите эти изменения на платформе **GitHub**, вы можете обновить свой локальный репозиторий до уровня удаленного командой:

```
git pull
```

Эта команда является _составной_ и состоит из двух других команд:
...
* `git fetch` - скачивает обновленные данные в локальный репозиторий
* `git merge` - делает Слияние
* `git pull` - делает сначала `git fetch`, а потом `git merge`.

Вы тоже можете поучавствовать в чужом проекте ровно так же, как и другие пользователи могут поучавствовать в вашем.

Для этого нужно сделать действия, которые мы уже знаем и некоторые другие:

1. Найти проект, в котором бы вы хотели поучавствовать, зайти в него и нажать на кнопку Fork:
![Fork](fork.jpg).
Эта кнопка копирует удаленный репозиторий к вам в ваш список удаленных репозиториев.
В появившемся окне мы можем переназвать копируемый репозиторий или оставить предлагаемое название, а так же добавить к нему комментарий.

2. Далее идем по уже известному пути, нажимаем Code:
![Code](code.jpg).

    Копируем ссылку и делаем `git clone <адрес>` в терминале вновь созданной папки.

3. Вносим свои изменения или предлагаем доработки и делаем **commit** об этом.

    * _**ВАЖНО!** Все изменения делаются в новой ветке!_

4. Потом выполняем команду `git push`. Так как изменения делаются из новой ветки, **Git** подсказывает дополнительную команду для `push`-а, которую мы применяем.

5. После этого переходим на страницу проекта, обновляем страницу и нажимаем:

    ![cdpr](candpr.jpg)

6. Убеждаемся, что статус в состоянии готовности без конфликтов:
![status](peredcpr.jpg)
...
8. Проверяем, что все пошло по плану:
![Code](poslecpr.jpg)

9. После этого у адресата появится поле для принятия ваших изменений с которым уже он будет работать.

10. Ждем принятия и радуемся, что мы смогли принять участие в проекте.

Бывает так, что после успешного `push`-а поле с кнопкой **Compare & pull request** не появляется на странице вашего Fork-а. Тогда необходимо нажать:

![Pullreq](pullreq.jpg)

и оформить свой **Pull request**.

Когда ваши правки/изменения были приняты или вы просто хотите обновить свой **Fork** до посленей версии, то на странице конкретного **Fork**-а в списке ваших репозиториев необходимо нажать: 

![fetchupstream](fetchupstream.jpg)

и выпадающем окне выбрать:

![fam](fam.jpg)

Ваш удаленный репозиторий с **Fork**-ом обновится и вам останется только применить команду `git pull`, что бы синхронизировать ваш локальный репозиторий с удаленным.

_**Особый случай:**_

Бывают такие ситуации, когда вам необходимо сделать **Pull request** в проект через связывание чужого удаленного репозитория с вашим и сохранить историю **commit**-ов. Тогда необходимо воспользоваться оператором для команды `git pull`:

```
--allow-unrelated-histories
```

Эта операция позволит вам связать вашу историю с историей проекта, в котором вы хотите поучавствовать.
